...
#Add missing standard repo
:<<'FIX'
echo -e '' >> /etc/apt/sources.list
echo -e "#Additional apt package locations (added by Collective Access Installer)" >> /etc/apt/sources.list
echo -e "deb http://ftp.ca.debian.org/debian stretch main" >> /etc/apt/sources.list
echo -e "deb-src http://ftp.ca.debian.org/debian stretch main" >> /etc/apt/sources.list
FIX
...
perl -pi -e "s/upload_max_filesize = 2M/upload_max_filesize = 30M/" /etc/php/7.0/apache2/php.ini
...
wget http://www.sno.phy.queensu.ca/~phil/exiftool/Image-ExifTool-10.59.tar.gz
...
then
	...
	$installString -y install autoconf automake build-essential libass-dev libfreetype6-dev libsdl1.2-dev libtheora-dev libtool libva-dev libvdpau-dev libvorbis-dev libxcb1-dev libxcb-shm0-dev libxcb-xfixes0-dev pkg-config texinfo zlib1g-dev yasm libx264-dev libfdk-aac-dev libmp3lame-dev libopus-dev cmake mercurial libcv-dev libvpx-dev

	mkdir /usr/local/ffmpeg_sources
	echo -e "\nBuilding libx265-dev..."
	...
	make install
	cd
	echo -e "Done building libx265-dev.\n"

	echo -e "Building ffmpeg..."
	cd /usr/local/ffmpeg_sources
...
echo -e "All done! All that remains is for you to run the installer from the webpage that will launch momentarily."
echo -e "Launching web installer..."
xdg-open "http://"$(myip)"/staff/install"
